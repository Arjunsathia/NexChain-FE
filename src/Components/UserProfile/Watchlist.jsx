...
function Watchlist() {
  ...
  const TC = useMemo(
    () => ({
      ...
      bgIcon: isLight
        ...
        : "bg-blue-500/10 text-blue-400", // Preserved Color
      iconColor: isLight ? "text-blue-600" : "text-cyan-400", // Preserved Color
      headerGradient:
        "bg-gradient-to-r from-cyan-400 to-blue-500 bg-clip-text text-transparent", // Preserved Color
      ...
      btnAction: isLight
        ? "text-blue-600 hover:text-blue-500 bg-gray-100 hover:bg-gray-200 border-none shadow-sm"
        : "text-cyan-400 hover:text-cyan-300 bg-gray-700/50 hover:bg-gray-600/50 border-none shadow-inner",

      borderFooter: isLight ? "border-gray-200" : "border-gray-800",
      textFooterLink: isLight
            ...
            {Array.from({ length: 4 }).map((_, idx) => (
              <div key={idx} className="flex items-center justify-between p-2">
                <div className="flex items-center gap-3 flex-1">
                  <div
                    className={`w-8 h-8 ${TC.bgSkeleton} rounded-full`}
                  ></div>
  ...
  return (
    ...
    >
      ...
      <div
        className={`absolute top-0 right-0 w-64 h-64 bg-cyan-500/5 rounded-full blur-3xl -mr-32 -mt-32 pointer-events-none transition-opacity duration-500 ${isLight ? "opacity-100" : "opacity-20"}`}
      />
          ...
          <div className="text-center py-8 flex flex-col items-center justify-center gap-3 h-full">
            <div className={`p-3 rounded-full ${TC.bgErrorIcon}`}>
              <FaExclamationTriangle
                className={`text-lg ${TC.textErrorIcon}`}
              />
            </div>
              ...
              >
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  ...
                  <div className="min-w-0 flex-1">
                    <span
                      className={`text-sm font-bold block truncate ${TC.textPrimary} ${TC.textItemHover}`}
                    >
                      {coin.name}
                    </span>
                    <span
                      className={`text-[10px] font-medium uppercase tracking-wider ${TC.textSecondary}`}
                    >
                      {coin.symbol}
                    </span>
                  </div>
                ...
                <div className="text-right">
                  <div
                    className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full inline-block mb-0.5 ${
      ...
      {!loading && !error && displayCoins.length > 0 && totalCoins > 4 && (
        <div
          className={`flex items-center justify-between pt-3 mt-1 border-t ${TC.borderFooter} text-xs ${TC.textSecondary} shrink-0 relative z-10`}
        >
