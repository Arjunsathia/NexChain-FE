...
function Portfolio() {
  ...
  const TC = useMemo(
    () => ({
      ...
      // Tooltip
      bgTooltip: isLight
        ? "bg-white/95 backdrop-blur-xl shadow-xl border border-gray-100"
        : "bg-gray-800/95 backdrop-blur-xl shadow-xl border border-gray-700",
    }),
    [isLight],
  );
  ...
  const portfolioData = useMemo(() => {
    ...
    let data = Object.values(portfolioMap).filter(
  ...
  const investedValue = portfolioData
    ...
    if (active && payload && payload.length) {
      ...
      const percentage =
        totalPortfolioValue > 0
          ? ((data.totalValue / totalPortfolioValue) * 100).toFixed(1)
          : 0;
      ...
      return (
        <div className={`${TC.bgTooltip} rounded-xl p-4 min-w-[180px]`}>
          <div className="flex items-center gap-2 mb-2">
            ...
            />
            <span className={`font-semibold text-sm ${TC.textPrimary}`}>
              {data.fullName}
            </span>
          </div>
          <div className="space-y-1 pl-5">
            ...
            <div
              className={`text-xs font-medium ${TC.textSecondary} flex justify-between`}
            >
              <span>{percentage}%</span>
              {data.type === "crypto" && (
                <span>
                  {data.quantity.toFixed(4)} {data.name}
                </span>
              )}
            </div>
          </div>
  ...
  return (
    ...
    >
      ...
      <div
        className={`absolute top-0 right-0 w-64 h-64 bg-blue-500/5 rounded-full blur-3xl -mr-32 -mt-32 pointer-events-none transition-opacity duration-500 ${isLight ? "opacity-100" : "opacity-20"}`}
      />
      ...
      <div className="flex items-center justify-between mb-4 z-10 shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div
              className={`text-xs font-medium flex items-center gap-1.5 ${TC.textSecondary}`}
            >
              <span className="font-semibold text-emerald-500 bg-emerald-500/10 px-1.5 py-0.5 rounded text-[10px]">
                +2.4%
              </span>
              <span>this week</span>
            </div>
        ...
        <div className="text-right">
          <p className={`text-xs font-medium mb-0.5 ${TC.textTertiary}`}>
            Total Value
          </p>
      ...
      {portfolioData.length === 0 ? (
        ...
        <div className="flex-1 min-h-0 flex flex-col">
          <div className="flex items-center h-[160px] shrink-0 mb-2">
            ...
            <div className="h-full w-1/2 relative">
              ...
              <div className="absolute inset-0 flex items-center justify-center pointer-events-none flex-col">
                <span className={`text-[10px] font-medium ${TC.textTertiary}`}>
                  Total
                </span>
            ...
            <div className="w-1/2 pl-2 space-y-2">
              <div
                className={`p-3 rounded-xl ${TC.bgStatItem} transition-all duration-200 hover:shadow-md cursor-default group/stat`}
              >
                <div className="flex items-center gap-2 mb-1">
                  <span
                    className={`p-1.5 rounded-lg bg-indigo-50/50 text-indigo-500 group-hover/stat:bg-indigo-500 group-hover/stat:text-white transition-colors duration-300`}
                  >
                    <FaCoins size={10} />
                  </span>
                  <span
                    className={`text-[10px] uppercase tracking-wider font-semibold ${TC.textTertiary}`}
                  >
                    Invested
                  </span>
                </div>
              ...
              >
                <div className="flex items-center gap-2 mb-1">
                  <span
                    className={`p-1.5 rounded-lg bg-emerald-50/50 text-emerald-500 group-hover/stat:bg-emerald-500 group-hover/stat:text-white transition-colors duration-300`}
                  >
                    <FaWallet size={10} />
                  </span>
                  <span
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar -mx-2 px-2 pb-2">
            <p
              className={`text-[10px] font-semibold uppercase tracking-wider mb-2 pl-1 ${TC.textTertiary}`}
            >
              Your Assets (
              {portfolioData.filter((i) => i.type !== "cash").length})
            </p>
            <div className="space-y-1">
              {portfolioData.map((item, index) => (
                <div
                  ...
                  className={`flex items-center justify-between p-2 rounded-xl transition-all duration-200 cursor-pointer group/item ${
                    activeIndex === index
                      ? isLight
                        ? "bg-blue-50"
                        : "bg-white/10"
                ...
                >
                  <div className="flex items-center gap-3 overflow-hidden">
                    <div
                      className="w-1.5 h-8 rounded-full shrink-0 transition-transform duration-300 group-hover/item:scale-y-110"
                      style={{ backgroundColor: item.color }}
                    />
                    <div className="min-w-0">
                      <p
                        className={`text-xs font-bold truncate ${TC.textPrimary}`}
                      >
                        {item.fullName}
                      </p>
                      <p
                      ...
                      >
                        {item.type === "cash"
                          ? "Fiat"
                          : `${item.quantity.toFixed(4)} ${item.name}`}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="text-right shrink-0">
                    ...
                    >
                      {totalPortfolioValue > 0
                        ? (
                            (item.totalValue / totalPortfolioValue) *
                            100
                          ).toFixed(1)
                        : 0}
                      %
