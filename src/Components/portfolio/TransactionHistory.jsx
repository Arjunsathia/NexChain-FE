...
const TransactionHistory = () => {
  ...
  const TC = useMemo(
    () => ({
      ...
      bgIcon: isLight
        ? "p-1.5 bg-blue-50 text-blue-600 rounded-lg"
        : "p-1.5 bg-blue-500/10 text-blue-400 rounded-lg",
      iconColor: isLight ? "text-blue-600" : "text-blue-400",

      bgFilterContainer: isLight
        ? "bg-gray-100/80 border-gray-200"
        : "bg-gray-800/50 border-gray-700/50",
      textFilterDefault: isLight
        ? "text-gray-500 hover:text-gray-900"
        : "text-gray-400 hover:text-white",

      bgBuySummary: isLight
        ? "bg-emerald-50/50 border-emerald-100"
        : "bg-emerald-500/5 border-emerald-500/10",
      bgSellSummary: isLight
...
const EmptyState = ({ TC }) => {
  return (
    <div className={`rounded-xl p-4 sm:p-6 fade-in ${TC.bgContainer}`}>
      <div
        className={`flex flex-col items-center justify-center py-8 fade-in ${TC.textPrimary}`}
      >
...
const Header = ({ TC, transactionCount, filter, onFilterChange }) => {
  const getFilterClass = (filterType) => {
    if (filter === filterType) {
      if (filterType === "all")
        return "bg-cyan-600 text-white shadow-lg shadow-cyan-500/20";
      if (filterType === "buy")
        return "bg-emerald-600 text-white shadow-lg shadow-emerald-500/20";
      if (filterType === "sell")
        return "bg-rose-600 text-white shadow-lg shadow-rose-500/20";
    }
  ...
  return (
    <div className="px-4 pt-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4 fade-in">
      <div className="flex items-center gap-3">
        ...
        <span
          className={`text-[10px] ${TC.textSecondary} px-2 py-0.5 rounded-full border border-white/5 font-bold uppercase tracking-wider`}
        >
          {transactionCount}
        </span>
      </div>
      ...
      <div className="flex items-center gap-2 fade-in">
        <div
          className={`flex rounded-lg p-0.5 border shadow-sm ${TC.bgFilterContainer}`}
        >
...
const TransactionSummary = ({
  ...
  return (
    <div className="grid grid-cols-2 gap-2 px-2 mb-4 fade-in">
      <div
        className={`border rounded-xl p-2 md:p-3 transition-all duration-200 fade-in ${TC.bgBuySummary}`}
      >
        ...
        <div className="flex items-end justify-between">
          <p className={`font-bold text-base ${TC.textQuantityBuy}`}>
            {buyCount}
          </p>
      ...
      <div
        className={`border rounded-xl p-2 md:p-3 transition-all duration-200 fade-in ${TC.bgSellSummary}`}
      >
        <div className="flex items-center justify-between mb-1">
          <p
            className={`text-[10px] font-bold uppercase tracking-wider ${TC.textSecondary} opacity-60`}
        ...
        <div className="flex items-end justify-between">
          <p className={`font-bold text-base ${TC.textQuantitySell}`}>
            {sellCount}
          </p>
          <p className={`text-[10px] font-bold ${TC.textSecondary}`}>
            $
...
const TransactionItem = ({ transaction, index, TC }) => {
  ...
  return (
    ...
    >
      <div
        className={`w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center flex-shrink-0 ${isBuy ? TC.bgBuyIcon : TC.bgSellIcon}`}
      >
        {isBuy ? (
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between mb-1">
          <div className="min-w-0 flex-1">
            ...
            <div className="flex items-center gap-2">
              <span
                className={`text-[10px] font-bold ${TC.textSecondary} opacity-60`}
              >
                {transaction.coinSymbol?.toUpperCase()}
              </span>
              <span
                className={`text-[9px] px-1.5 py-0.5 rounded-full font-bold uppercase ${isBuy ? TC.bgPillBuy : TC.bgPillSell}`}
              >
                {isBuy ? "Bought" : "Sold"}
              </span>
            </div>
          ...
          <div className="text-right">
            <p
              className={`font-bold text-sm ${isBuy ? TC.textQuantityBuy : TC.textQuantitySell}`}
            >
              {isBuy ? "+" : "-"}
        ...
        <div
          className={`flex items-center justify-between text-[10px] font-medium pt-1.5 border-t ${TC.borderItem}`}
        >
          ...
          <span className={`flex items-center gap-1 ${TC.textDateAccent}`}>
            ...
            {formatDateTime(
              transaction.transactionDate || transaction.purchase_date,
            )}
          </span>
        </div>
      </div>
