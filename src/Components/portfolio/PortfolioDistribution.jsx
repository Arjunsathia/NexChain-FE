    ...
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p
              className={`text-[10px] font-bold uppercase tracking-wider mb-0.5 ${TC.textSecondary} opacity-60`}
            >
              Total P&L (Crypto)
            </p>
        ...
        <div className="text-right">
          <p className={`text-base font-bold ${textPL}`}>
            {isPositive ? "+" : ""}
            {totalProfitLossPercentage?.toFixed(2) || "0.00"}%
          </p>
        </div>
      </div>
    </div>
  ...
  if (active && payload && payload.length) {
    ...
    const percentage =
      totalPortfolioValue > 0
        ? ((data.value / totalPortfolioValue) * 100).toFixed(1)
        : 0;
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between text-xs">
        <div
          className={`flex items-center gap-2 ${TC.textSecondary} font-bold uppercase tracking-wider opacity-60`}
        >
      ...
      <div
        className={`flex items-center justify-between text-xs pt-2 border-t ${TC.borderSummary}`}
      >
        <div
          className={`flex items-center gap-2 font-bold uppercase tracking-wider ${TC.textSecondary} opacity-60`}
        >
      ...
      {groupedHoldings.length > 0 && (
        <div
          className={`flex items-center justify-between text-xs ${TC.textSecondary} pt-2`}
        >
