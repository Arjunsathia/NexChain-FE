...
const PriceAlerts = ({ isLight }) => {
  ...
  return (
    <div
      className={`rounded-2xl border p-6 ${TC.bgCard} shadow-sm fade-in mt-6`}
    >
      <h3
      ...
      <div className="space-y-3">
        {alerts.map((alert) => (
          <div
            key={alert._id}
            className={`flex items-center justify-between p-3 rounded-xl border border-transparent ${TC.rowHover} transition-all`}
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <div
                  className={`font-bold ${TC.textPrimary} flex items-center gap-2`}
                >
                  {alert.coin_symbol.toUpperCase()}
                  <span
                    className={`text-xs px-2 py-0.5 rounded-full ${alert.condition === "above" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
                  >
