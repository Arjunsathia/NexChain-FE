  ...
  const handleExecute = useCallback(
    async (orderId, currentPrice) => {
      ...
      try {
        ...
        if (res.data.success) {
          toast.success(
            `Order Executed: ${res.data.order.coin_symbol.toUpperCase()}`,
          );
          fetchOrders();
        }
      } catch (err) {
  ...
  useEffect(() => {
    ...
    orders.forEach((order) => {
      ...
      if (priceData && priceData.current_price) {
        ...
        if (
          order.category === "stop_limit" ||
          order.category === "stop_market"
        ) {
          if (
            (order.type === "buy" && currentPrice >= order.stop_price) ||
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            ...
            >
              ...
              <th className={`py-4 text-right pr-4 ${TC.textSecondary}`}>
                Action
              </th>
            </tr>
          </thead>
          ...
          >
            {orders.map((order) => (
              <tr
                key={order._id}
                className={`transition-colors text-sm ${TC.bgHover}`}
              >
                ...
                <td className={`py-3 font-bold ${TC.textPrimary}`}>
                  {order.stop_price
