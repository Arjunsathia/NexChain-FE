...
const PerformanceChart = ({ isLight, groupedHoldings, balance, loading }) => {
  ...
  const TC = useMemo(() => ({
    ...
    tooltipBg: isLight ? "bg-white border border-gray-300 shadow-xl" : "bg-gray-900 border border-gray-800 shadow-2xl",
...
const Header = ({ timeRange, setTimeRange, TC }) => {
  return (
    <div className="px-4 pt-3 flex items-center justify-between mb-2">
      ...
      <div className={`flex rounded-lg p-0.5 border shadow-sm ${TC.bgRangeContainer}`}>
        {['7d', '30d', '90d'].map((range) => (
          <button
            key={range}
            onClick={() => setTimeRange(range)}
            className={`px-2 py-0.5 text-[10px] font-bold rounded-md transition-all ${timeRange === range ? TC.bgRangeButtonActive : TC.bgRangeButtonDefault
              }`}
          >
            {range}
          </button>
