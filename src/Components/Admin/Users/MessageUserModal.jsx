        ...
        <div className="fixed inset-0 z-[100000] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
            ...
            >
                ...
                <div className={`px-6 py-4 border-b flex justify-between items-center ${isLight ? "border-gray-100" : "border-gray-800"}`}>
                    <h3 className={`text-lg font-bold flex items-center gap-2 ${TC.textPrimary}`}>
                ...
                <form onSubmit={handleSend} className="p-6 space-y-4">
                    ...
                    {/* User Info Preview */}
                    <div className={`flex items-center gap-3 p-3 rounded-xl border ${isLight ? "bg-gray-50 border-gray-200" : "bg-white/5 border-white/10"}`}>
                        ...
                        <div>
                            <p className={`text-sm font-bold ${TC.textPrimary}`}>{user.name}</p>
                    ...
                    {messageType === "email" && (
                        <div className="space-y-1">
                            <label className={`text-xs font-bold uppercase tracking-wider ${TC.textSecondary}`}>Subject</label>
                            <input
                                type="text"
                                value={subject}
                    ...
                    {/* Message Content */}
                    <div className="space-y-1">
                        <label className={`text-xs font-bold uppercase tracking-wider ${TC.textSecondary}`}>Message</label>
                        <textarea
                            value={message}
                    ...
                    <div className="flex gap-3 pt-2">
                        <button
                            ...
                            onClick={onClose}
                            disabled={loading}
                            className={`flex-1 py-2.5 rounded-xl font-bold text-sm transition-colors ${isLight ? "bg-gray-100 text-gray-600 hover:bg-gray-200" : "bg-white/5 text-gray-300 hover:bg-white/10"
                                }`}
                        >
                            Cancel
                        </button>
                        <button
                            type="submit"
                            disabled={loading}
                            className={`flex-1 py-2.5 rounded-xl font-bold text-sm text-white shadow-lg shadow-blue-500/25 flex items-center justify-center gap-2 transition-all hover:scale-[1.02] active:scale-95 ${loading ? "bg-blue-600/50 cursor-not-allowed" : "bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500"
                                }`}
                        >
                            {loading ? (
                                <>Sending...</>
                                ...
                                <>
                                    <Send className="w-4 h-4" /> Send {messageType === "email" ? "Email" : "Message"}
                                </>
                            )}
                        </button>
                    </div>
