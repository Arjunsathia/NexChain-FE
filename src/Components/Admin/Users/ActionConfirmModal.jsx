...
const ActionConfirmModal = ({ isOpen, onClose, onConfirm, actionType, userName, loading, TC }) => {
    ...
    return (
        <Dialog open={isOpen} as="div" className="relative z-[99999]" onClose={onClose} transition>
            <DialogBackdrop
                ...
                <div className="flex min-h-full items-center justify-center p-4 text-center">
                    ...
                    >
                        <div className="flex items-center justify-between mb-4">
                            <div className={`p-3 rounded-full ${isArchive ? 'bg-orange-500/10' : isRestore ? 'bg-green-500/10' : 'bg-blue-500/10'}`}>
                                {icon}
                            </div>
                            <button onClick={onClose} className={`p-1 rounded-lg hover:bg-gray-100 dark:hover:bg-white/5 ${TC.textSecondary}`}>
                                <X className="w-5 h-5" />
                            </button>
                        </div>

                        <DialogTitle as="h3" className={`text-lg font-bold leading-6 ${TC.textPrimary} mb-2`}>
                            {title}
                        </DialogTitle>
                        <div className="mt-2">
