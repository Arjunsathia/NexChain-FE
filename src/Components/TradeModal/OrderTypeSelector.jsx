...
const OrderTypeSelector = ({ orderType, setOrderType, setIsAlertMode, isBuyOperation }) => {
    ...
    return (
        <div className="relative mb-6 z-20" ref={dropdownRef}>
            <label className={`block text-[10px] font-bold uppercase tracking-wider mb-1.5 ${isLight ? "text-gray-500" : "text-gray-400"}`}>
                Order Type
            </label>
            ...
            >
                ...
                <FaChevronDown className={`text-xs transition-transform duration-200 ${isOpen ? "rotate-180" : ""} ${isLight ? "text-gray-400" : "text-gray-500"}`} />
            </button>
                    ...
                    return (
                        <button
                            ...
                            className={`
                                ...
                                ${isActive
                                    ? (isLight ? "bg-gray-50 text-gray-900" : "bg-white/5 text-white")
                                    : (isLight ? "text-gray-600 hover:bg-gray-50" : "text-gray-400 hover:bg-white/5 hover:text-gray-200")
                                }
                            `}
                        >
                            <span>{formatType(type)}</span>
                            {isActive && (
                                <div className={`w-1.5 h-1.5 rounded-full ${isBuyOperation ? "bg-emerald-500" : "bg-rose-500"}`} />
                            )}
                        </button>
                    );
                })}
