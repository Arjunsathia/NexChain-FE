...
const HoldingsInfo = React.memo(({ holdingsSummary, activeTab, isLight, symbol, currentPrice, coin, setActiveTab }) => {
  ...
  return (
    <div className="mb-4 transition-all duration-300">
      ...
      >
        {activeTab === 'details' && (
          <div className={`px-4 py-3 border-b flex items-center justify-between ${isLight ? "border-gray-200 bg-gray-50/50" : "border-white/5 bg-white/5"}`}>
            <h3 className={`text-xs font-bold flex items-center gap-1.5 ${isLight ? "text-cyan-700" : "text-cyan-400"}`}>
        ...
        <div className="grid grid-cols-3 gap-px bg-gray-200 dark:bg-white/5">
          {/* Qty */}
          <div className={`p-3 flex flex-col ${isLight ? "bg-white" : "bg-gray-900"}`}>
            <span className={`text-[9px] uppercase font-bold tracking-wider mb-0.5 ${isLight ? "text-gray-400" : "text-gray-500"}`}>Qty Owned</span>
            <div className={`text-xs font-bold ${isLight ? "text-gray-900" : "text-white"}`}>
              {holdingsSummary.totalQuantity.toLocaleString("en-US", { maximumFractionDigits: 8 })}
            </div>
          </div>

          {/* Value */}
          <div className={`p-3 flex flex-col ${isLight ? "bg-white" : "bg-gray-900"}`}>
            <span className={`text-[9px] uppercase font-bold tracking-wider mb-0.5 ${isLight ? "text-gray-400" : "text-gray-500"}`}>Current Value</span>
            <div className={`text-xs font-bold ${isLight ? "text-gray-900" : "text-white"}`}>
              ${holdingsSummary.currentValue.toLocaleString("en-IN", { maximumFractionDigits: 0 })}
            </div>
          </div>

          {/* Avg */}
          <div className={`p-3 flex flex-col ${isLight ? "bg-white" : "bg-gray-900"}`}>
            <span className={`text-[9px] uppercase font-bold tracking-wider mb-0.5 ${isLight ? "text-gray-400" : "text-gray-500"}`}>Avg Price</span>
            <div className={`text-xs font-bold ${isLight ? "text-gray-900" : "text-white"}`}>
              ${holdingsSummary.averagePrice.toLocaleString("en-IN", { maximumFractionDigits: 0 })}
            </div>
          </div>

          {/* Invested */}
          <div className={`p-3 flex flex-col ${isLight ? "bg-white" : "bg-gray-900"}`}>
            <span className={`text-[9px] uppercase font-bold tracking-wider mb-0.5 ${isLight ? "text-gray-400" : "text-gray-500"}`}>Invested</span>
            <div className={`text-xs font-bold ${isLight ? "text-gray-900" : "text-white"}`}>
              ${holdingsSummary.remainingInvestment.toLocaleString("en-IN", { maximumFractionDigits: 0 })}
            </div>
          </div>

          {/* Net P&L */}
          <div className={`p-3 flex flex-col ${isLight ? "bg-white" : "bg-gray-900"}`}>
            <span className={`text-[9px] uppercase font-bold tracking-wider mb-0.5 ${isLight ? "text-gray-400" : "text-gray-500"}`}>Net P&L</span>
            <div className={`text-xs font-bold ${holdingsSummary.profitLoss >= 0 ? "text-green-500" : "text-red-500"}`}>
              {holdingsSummary.profitLoss >= 0 ? "+" : ""}${Math.abs(holdingsSummary.profitLoss).toLocaleString("en-IN", { maximumFractionDigits: 0 })}
            </div>
          </div>

          {/* P&L % */}
          <div className={`p-3 flex flex-col ${isLight ? "bg-white" : "bg-gray-900"}`}>
            <span className={`text-[9px] uppercase font-bold tracking-wider mb-0.5 ${isLight ? "text-gray-400" : "text-gray-500"}`}>P&L %</span>
            <div className={`text-xs font-bold ${holdingsSummary.profitLoss >= 0 ? "text-green-500" : "text-red-500"}`}>
        ...
        <div className="animate-in fade-in slide-in-from-top-4 duration-300 space-y-4">
          ...
          <div className={`p-4 rounded-xl border ${isLight ? "bg-blue-50/50 border-blue-100" : "bg-gray-800 border-gray-700"}`} >
            ...
            <div className={`mt-3 flex items-start gap-1.5 text-[10px] leading-tight ${isLight ? "text-gray-500" : "text-gray-400"}`}>
              ...
              <p>Net P&L shows your profit/loss in USD ($), while P&L % shows your percentage return on investment.</p>
            </div>
          </div>
