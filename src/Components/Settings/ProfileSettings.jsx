...
import { FaUser, FaEnvelope, FaLock, FaSave, FaCamera, FaTimes, FaSearchPlus, FaSearchMinus, FaPhone, FaShieldAlt } from "react-icons/fa";
  ...
  return (
    <>
      <form onSubmit={handleSubmit} className="space-y-8 animate-in fade-in duration-300">
        ...
        <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6 sm:gap-8 pb-8 border-b border-dashed" style={{ borderColor: isLight ? '#e5e7eb' : 'rgba(255,255,255,0.1)' }}>
          <div className="relative group cursor-pointer shrink-0" onClick={() => !isUploading && fileInputRef.current.click()}>
            ...
            />
            <div className={`w-28 h-28 rounded-2xl overflow-hidden border-2 flex items-center justify-center relative shadow-lg ${isLight ? "border-white bg-gray-100 shadow-blue-500/10" : "border-gray-700 bg-gray-800 shadow-black/40"}`}>
              {user?.image ? (
                <img
                ...
                <span className={`text-4xl font-bold ${isLight ? "text-blue-600" : "text-cyan-400"}`}>
          ...
          <div className="text-center sm:text-left pt-2">
            <h2 className={`text-2xl font-bold ${TC.textPrimary} mb-1 tracking-tight`}>{user?.name}</h2>
            <p className={`font-medium ${TC.textSecondary} mb-3`}>@{user?.user_name}</p>
            <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-lg text-[10px] uppercase font-bold tracking-wider ${isLight ? "bg-blue-50 text-blue-600" : "bg-blue-500/10 text-blue-400"}`}>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div className="space-y-1.5">
              <label className={`text-xs font-bold uppercase tracking-wider ${TC.textSecondary} ml-1`}>Username</label>
              <div className="relative group">
                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 font-bold group-focus-within:text-cyan-500 transition-colors">@</span>
                <input
                  type="text"
                  name="user_name"
            ...
            <div className="space-y-1.5">
              <label className={`text-xs font-bold uppercase tracking-wider ${TC.textSecondary} ml-1`}>Email Address</label>
              <div className="relative group">
            ...
            <div className="space-y-1.5">
              <label className={`text-xs font-bold uppercase tracking-wider ${TC.textSecondary} ml-1`}>Phone Number</label>
              <div className="relative group">
        ...
        {/* Section 3: Security */}
        <div className={`pt-6 border-t border-dashed`} style={{ borderColor: isLight ? '#e5e7eb' : 'rgba(255,255,255,0.1)' }}>
          <h3 className={`text-lg font-bold mb-5 flex items-center gap-2 ${TC.textPrimary} tracking-tight`}>
            Security Settings
          </h3>
          ...
          <div className="space-y-5">
            <div className="space-y-1.5">
              <label className={`text-xs font-bold uppercase tracking-wider ${TC.textSecondary} ml-1`}>Current Password</label>
              <div className="relative group">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div className="space-y-1.5">
                <label className={`text-xs font-bold uppercase tracking-wider ${TC.textSecondary} ml-1`}>New Password</label>
                <div className="relative group">
                  <FaLock className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-cyan-500 transition-colors" />
                  <input
                    type="password"
              ...
              <div className="space-y-1.5">
                <label className={`text-xs font-bold uppercase tracking-wider ${TC.textSecondary} ml-1`}>Confirm Password</label>
                <div className="relative group">
                  <FaLock className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-cyan-500 transition-colors" />
                  <input
                    type="password"
        ...
        <div className="flex flex-col-reverse sm:flex-row justify-end gap-3 pt-6 border-t border-dashed" style={{ borderColor: isLight ? '#e5e7eb' : 'rgba(255,255,255,0.1)' }}>
          <button
            type="submit"
            disabled={isSaving}
      ...
      {isCropping && createPortal(
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className={`w-full max-w-lg rounded-3xl overflow-hidden shadow-2xl ${isDark ? "bg-gray-900 border border-gray-800" : "bg-white"}`}>
            <div className={`p-5 flex justify-between items-center border-b ${isDark ? "border-gray-800" : "border-gray-100"}`}>
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="flex gap-3">
                <button
                  onClick={() => { setIsCropping(false); setImageSrc(null); }}
                  className={`flex-1 py-3 rounded-xl font-bold transition-colors ${isDark ? "bg-gray-800 text-gray-300 hover:bg-gray-700" : "bg-gray-100 text-gray-700 hover:bg-gray-200"
                    }`}
                >
                  Cancel
                </button>
