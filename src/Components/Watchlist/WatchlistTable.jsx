...
const WatchlistTable = ({ coins, TC, isLight, handleCoinClick, handleTrade, setRemoveModal, handleAlertClick, disableAnimations = false }) => {
  ...
  return (
    <div className={`hidden md:block p-1 rounded-xl ${shouldAnimate ? 'fade-in' : ''} ${TC.bgCard}`} style={shouldAnimate ? { animationDelay: "0.2s" } : {}}>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className={`${isLight ? "bg-gray-100/50" : "bg-white/5"} uppercase tracking-wider text-[10px] font-bold`}>
              ...
              <th className={`py-4 px-6 text-right ${TC.textSecondary}`}>24h Change</th>
              <th className={`py-4 px-6 text-right ${TC.textSecondary}`}>Market Cap</th>
              <th className={`py-4 px-6 text-center ${TC.textSecondary}`}>7d Trend</th>
              <th className={`py-4 px-6 text-center ${TC.textSecondary}`}>Holdings</th>
              <th className={`py-4 px-6 text-center ${TC.textSecondary}`}>Actions</th>
            </tr>
          </thead>
            ...
            {coins.map((coin, index) => (
              ...
              >
                ...
                <td className="py-4 px-6">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                      <div className={`font-bold text-base truncate group-hover:text-cyan-400 transition-colors ${TC.textPrimary}`}>
                        {coin.name}
                      </div>
                ...
                <td className="py-4 px-6 text-right">
                  <div className={`inline-flex items-center gap-1 px-3 py-1.5 rounded-lg font-bold text-sm ${(coin.price_change_percentage_24h || 0) >= 0
                    ? `${isLight ? "bg-emerald-100" : "bg-emerald-500/20"} ${TC.textPositive}`
                    : `${isLight ? "bg-rose-100" : "bg-red-500/20"} ${TC.textNegative}`
                    }`}>
                    <span className="text-xs">{(coin.price_change_percentage_24h || 0) >= 0 ? "↑" : "↓"}</span>
                ...
                <td className="py-4 px-6 text-center">
                  {coin.userHolding ? (
                    <div className={`inline-flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-semibold ${isLight ? "bg-green-100 text-green-700" : "bg-green-500/20 text-green-400"
                      }`}>
                ...
                <td className="py-4 px-6" onClick={(e) => e.stopPropagation()}>
                  <div className="flex items-center justify-center gap-2">
                    ...
                    <button
                      ...
                      className={`p-2 rounded-xl border transition-all duration-200 ${isLight ? "bg-gray-100/50 text-red-600 hover:bg-red-600 hover:text-white border-gray-200" : "bg-white/5 text-red-400 hover:bg-red-600 hover:text-white border-white/5"
                        }`}
                    >
      ...
      { }
      <div className={`px-6 py-4 border-t ${isLight ? "bg-gray-50/50 border-gray-100" : "bg-white/5 border-white/5"}`}>
