...
const PurchaseSuccessModal = ({
  ...
  return (
    <div
      className={`fixed inset-0 z-[60] flex items-center justify-center bg-black/80 backdrop-blur-sm transition-opacity duration-300 ${show ? "opacity-100" : "opacity-0 pointer-events-none"}`}
    >
      {isFirstPurchase && (
        <Confetti
      ...
      <div
        className={`relative w-full max-w-md mx-4 rounded-3xl shadow-2xl overflow-hidden transform transition-all duration-300 ${show ? "scale-100 opacity-100" : "scale-95 opacity-0"} ${
          isLight
            ? "bg-white"
