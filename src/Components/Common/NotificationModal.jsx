...
import { FaBell, FaTrash, FaTimes, FaInfoCircle, FaExclamationTriangle, FaCheckCircle, FaExclamationCircle } from 'react-icons/fa';
    ...
    <AnimatePresence>
      {isOpen && (
        <div style={{ display: 'contents' }}>
          ...
          >
            { }
            <div className={`p-3 md:p-4 border-b flex justify-between items-center ${isDark ? 'border-gray-700/50 bg-gray-900/50' : 'border-gray-100 bg-white/50'
              }`}>
              <div className="flex items-center gap-2">
                ...
                <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold ${isDark ? 'bg-cyan-500/20 text-cyan-400' : 'bg-cyan-100 text-cyan-700'
                  }`}>
            ...
            <div className={`max-h-[60vh] md:max-h-[450px] overflow-y-auto custom-scrollbar bg-transparent`}>
              {loading ? (
                ...
                <div className="p-12 text-center flex flex-col items-center justify-center text-gray-400">
                  ...
                  <p className={`font-medium mb-1 ${isDark ? 'text-white' : 'text-gray-900'}`}>No notifications yet</p>
                  ...
                  {notifications.map((notification) => (
                    <div
                      ...
                      className={`p-4 transition-colors cursor-pointer relative group ${isDark ? 'hover:bg-gray-800/50' : 'hover:bg-gray-50'
                          ...
                          ? (isDark ? 'bg-cyan-900/10' : 'bg-cyan-50/40')
                      ...
                      <div className="flex gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex justify-between items-start gap-2 mb-1">
                            <h4 className={`text-sm font-semibold ${isDark ? 'text-white' : 'text-gray-900'} ${notification.isRead ? (isDark ? 'text-gray-300' : 'text-gray-700') : ''}`}>
                              {notification.title}
                            </h4>
                            <span className={`text-[10px] text-gray-400 whitespace-nowrap px-1.5 py-0.5 rounded ${isDark ? 'bg-gray-800' : 'bg-gray-100'}`}>
