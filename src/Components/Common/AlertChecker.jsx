    ...
    try {
      ...
      ws.current.onmessage = (event) => {
        ...
        if (message.stream && message.data) {
          const symbol = message.stream
            .replace("@ticker", "")
            .replace("usdt", "");
          const price = parseFloat(message.data.c);

          const relevantAlerts = alerts.filter(
            (a) => a.coin_symbol.toLowerCase() === symbol,
          );
    ...
    const checkTriggers = async () => {
      ...
      alerts.forEach((alert) => {
        if (livePrices[alert.coin_id]) {
          pricesToCheck[alert.coin_id] =
            livePrices[alert.coin_id].current_price;
        }
      });
            ...
            res.data.triggered.forEach((alert) => {
              toast(
                ...
                ),
                {
                  ...
                  position: "top-right",
                  className:
                    "!max-w-[85vw] !p-2 md:!p-4 !bg-white !text-gray-900 !border !border-gray-200 !shadow-md !rounded-xl",
                },
              );
            });
