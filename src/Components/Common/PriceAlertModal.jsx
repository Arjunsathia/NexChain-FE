...
const PriceAlertModal = ({ show, onClose, coin }) => {
        ...
        toast.success(
          <div>
            ...
            <div className="text-xs font-normal opacity-90 mt-1">
              You&apos;ll be notified when {symbol.toUpperCase()} goes{" "}
              {condition} ${priceValue}
            </div>
          </div>,
        );
        onClose();
  ...
  const percentDiff =
    currentPrice > 0 ? ((priceValue - currentPrice) / currentPrice) * 100 : 0;
  ...
  return createPortal(
    ...
    >
      ...
      >
        ...
        >
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span
                className={`text-xs px-2 py-0.5 rounded-full font-medium ${isLight ? "bg-gray-200 text-gray-600" : "bg-gray-700 text-gray-300"}`}
              >
        ...
        <div className="mb-6">
          <label
            className={`block text-xs font-bold uppercase tracking-wider mb-2 ${TC.textSecondary}`}
          >
            Target Price (USD)
          </label>
          <div className="relative">
            <div
              className={`absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-lg ${TC.textSecondary}`}
            >
              $
            </div>
            <input
        ...
        >
          {isSubmitting ? (
            ...
            <>
              <FaBell />
              Create Alert for $
              {priceValue ? parseFloat(priceValue).toLocaleString() : "0.00"}
            </>
          )}
        </button>
      </div>
