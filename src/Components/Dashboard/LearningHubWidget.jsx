...
export default function LearningHubWidget() {
  ...
  return (
    <div className={`rounded-xl p-3 h-full flex flex-col transition-all duration-300 ease-in-out hover:shadow-lg ${TC.bgCard}`}>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar flex flex-col gap-2 pr-1">
          {[
            ...
            <div key={i} className="flex items-start gap-2 flex-shrink-0">
              <div className={`w-1.5 h-1.5 mt-1.5 rounded-full ${isLight ? "bg-blue-400" : "bg-blue-500"}`}></div>
              <div className={`flex-1 p-2 rounded-lg ${isLight ? "bg-gray-50 border border-gray-100" : "bg-gray-800/50 border border-gray-700/30"}`}>
                <p className={`text-[11px] leading-relaxed font-medium ${TC.textSecondary}`}>
                  {text}
                </p>
              </div>
            </div>
