...
export const useBinanceTickerContext = () => {
    ...
    if (!context) {
        throw new Error('useBinanceTickerContext must be used within a BinanceTickerProvider');
    }
    return context;
};
...
const symbolMap = {
    ...
    "usd-coin": "usdcusdt", "tether": "usdtusdt", "arbitrum": "arbusdt", "optimism": "opusdt",
    "near": "nearusdt", "aptos": "aptusdt", "fantom": "ftmusdt"
};
...
export const BinanceTickerProvider = ({ children }) => {
    ...
    useEffect(() => {
        ...
        const connect = () => {
            ...
            const streams = Object.values(symbolMap).map(s => `${s}@ticker`).join('/');
