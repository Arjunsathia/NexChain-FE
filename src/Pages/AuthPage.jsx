...
const AuthPage = () => {
    ...
    const googleLogin = useGoogleLogin({
        onSuccess: async (tokenResponse) => {
            ...
            try {
                ...
                const res = await postForm("/auth/google", { googleAccessToken: tokenResponse.access_token });
                ...
                if (res.accessToken) {
                    ...
                    await fetchUsers();
                    toast.success(`Welcome back ${res.user.name}!`, { id: "google-auth" });
                    navigate("/dashboard");
                }
            } catch (err) {
                console.error("Google Login Error", err);
                toast.error(err.response?.data?.message || "Google Login failed", { id: "google-auth" });
            }
        },
    ...
    return (
        <div className="relative min-h-screen w-full bg-transparent text-white flex items-center justify-center overflow-hidden font-sans selection:bg-cyan-500/30">
            ...
            <div className="relative z-10 w-full max-w-[1000px] px-4 md:px-6 grid xl:grid-cols-2 gap-8 items-center justify-items-center">
